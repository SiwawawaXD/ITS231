...
public class Sorting {
	...
	private static void RecursiveMergeSort(int[] A) { // Ex3 Complete method RecursiveMergeSort
		...
		for (int i = 0; i < midIndex; i++) { //LeftHalf
		...
		for (int i = midIndex; i < inputLength; i++) { //RightHalf
			//System.out.println(i);
			if(i - midIndex != rightHalf.length-1) {
				if(A[i]>A[i+1]) {
					swap(A,i,i+1);
				}
			}
	...
	static void RecursiveQuickSort(int[] A, int start, int end) { // Ex4. Complete the method RecursiveQuickSort
		if (start < end) {
			...
			for (int i = 0; i < r_count; i++) {
				if(R[i] > R[i+1] && i != r_count - 1) {
					swap(R, i, i+1);
				}
				A[l_count + start + i + 1] = R[i];
			}
			//printArray(A);
			RecursiveQuickSort(A, start, end-1);
		}
